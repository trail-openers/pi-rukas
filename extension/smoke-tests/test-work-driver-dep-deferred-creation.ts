...
// -------------------------------------------------------- case 2: creation fails
{
  ...
  );
  {
    ...
    parked.pipelineState.currentStep = "develop"; // still the in-flight step
    const decision = nextStep(parked);
    assert(
      decision.kind === "step" && decision.step === "handoff",
      "#753 case 2: nextStep routes the parked state to handoff on the cap-hit (the cap-hit is the tail)",
    );
  }
