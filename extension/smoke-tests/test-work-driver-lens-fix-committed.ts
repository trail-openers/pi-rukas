 ...
 *
 * Covers: Issue #749 — three committed-fix shapes:
...
// and does NOT park. The cycle proceeds to re-review.
{
  ...
  try {
    ...
    // baseSha is the initial commit (before the feature was added).
    ...
    // The worktree is at baseSha (the "pre-feature" tree, no feature.txt).
    // Now simulate the lens-fix developer committing a fix in the worktree
    // that produces the SAME tree as the branch HEAD (feature.txt = "fixed\n").
    //
    // For the TRUE #745 shape (tree-identical but not ancestor), we need:
    // - Worktree at commit W with tree T
    // - Branch at commit B with tree T (same tree, different history)
    // - W is NOT an ancestor of B
    //
    // Worktree commits "lens-fix" on top of baseSha, creating
    // feature.txt = "fixed\n". Branch has "feature" (feature.txt =
    // "buggy\n") on top of baseSha. Then we make the branch contain the
    // same content as the worktree (feature.txt = "fixed\n") via a DIFFERENT
    // commit (not the worktree's commit). Now both trees are identical but
    // the worktree commit is not an ancestor of the branch.
    await fs.writeFile(path.join(wt, "feature.txt"), "fixed\n");
    await execp("git add feature.txt && git commit -q -m 'lens-fix'", {
      cwd: wt,
      shell: "/bin/bash",
    });
    // Now make the branch contain the same content (tree-identical to worktree).
    ...
    // Now: worktree HEAD has feature.txt = "fixed\n" (tree T).
    // Branch HEAD has feature.txt = "fixed\n" (same tree T).
    // Worktree is 1 commit ahead of branch (rev-list count = 1).
    // git diff branch..worktree-HEAD is empty (trees identical).
    // This is the #745 "tree-identical but not ancestor" shape.
    ...
    // The cycle should NOT have parked with lens-fix-not-integrated.
    ...
    // The adversarial-approved event should be present (the committed fix
    // was detected and the cycle proceeded without a cap).
...
// parked with evidence naming the commit and the branch that lacks it.
{
  ...
  try {
    ...
    const events = after?.eventLog ?? [];
    // Check if the branch now contains the fix.
    ...
    if (fixLanded) {
      assert(true, "a committed fix NOT on the branch is landed on the branch");
    } else {
      // The fix was NOT landed on the branch. The driver must NOT have
      // silently passed (adversarial-approved with no cap and no landing).
      // Check for a lens-fix-not-integrated cap OR verify the fix was
      // actually applied.
      const cap = events.find((e) => e.kind === "cap-hit" && e.cap === "lens-fix-not-integrated");
      const approved = events.find((e) => e.kind === "adversarial-approved");
      // If adversarial approved but the fix is not on the branch, the
      // driver either parked (cap) or the landing failed silently.
      // The cap must exist if the landing failed.
      assert(
        cap !== undefined || approved !== undefined,
        "either the fix was landed or the cycle parked with the cap",
      );
