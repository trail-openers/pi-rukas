...

{
  ...
  assert(claimTokens("a,b c").has("a") && claimTokens("a,b c").has("b") && claimTokens("a,b c").size === 3, "tokens: split + no empties");
...

{
  ...
  assert(jaccardSimilarity(a, b) < DEDUP_JACCARD_THRESHOLD, "jaccard: 17/22 below the threshold (no merge)");
  ...
  const c = `${s2} p1 p2`;
  const d = `${s2} p1 p2 q3`;
  assert(jaccardSimilarity(c, d) >= DEDUP_JACCARD_THRESHOLD, "jaccard: 35/38 above the threshold (merge)");

  const merged = dedupResearchClaims([mk({ text: c, source: "https://x/doc" }), mk({ text: d, source: "https://x/doc" })]);
  assert(merged.length === 1, "merge: same normalised source + jaccard ≥ 0.85 → one claim");
  const notMerged = dedupResearchClaims([mk({ text: a, source: "https://x/doc" }), mk({ text: b, source: "https://x/doc" })]);
...

{
  ...
  assert(jaccardSimilarity(lo, loB) < DEDUP_JACCARD_THRESHOLD, "boundary: 5/12 sits BELOW → no merge");
  assert(jaccardSimilarity(hi, hiB) >= DEDUP_JACCARD_THRESHOLD, "boundary: 0.85 sits exactly AT → merge");

  const loMerge = dedupResearchClaims([mk({ text: lo, source: "https://x/d" }), mk({ text: loB, source: "https://x/d" })]);
  const hiMerge = dedupResearchClaims([mk({ text: hi, source: "https://x/d" }), mk({ text: hiB, source: "https://x/d" })]);
  ...
  assert(hiMerge[0]?.angles?.join(",") === "a,a" || hiMerge[0]?.angles?.length === 1, "boundary: survivor's angles recorded");
}
...

{
  ...
  const out = dedupResearchClaims([
    mk({ text: "Vipune hybrid scores ARE RRF reciprocals!", angle: "web-current", source: "https://a/x" }),
    mk({ text: "vipune hybrid scores are RRF reciprocals.", angle: "docs-depth", source: "https://a/x" }),
  ]);
  assert(out.length === 1, "identical normalised text merges across angles");
  assert(out[0]?.text === "Vipune hybrid scores ARE RRF reciprocals!", "survivor: first-encountered text wins the tie");
  assert(out[0]?.angle === "web-current", "survivor: angle field stays the survivor's");
  assert(out[0]?.angles?.join(",") === "web-current,docs-depth", "angles: all contributors, deduped, first-appearance order");
...

{
  ...
  const out = dedupResearchClaims([
    mk({ text: "same text", source: "https://a/x", confidence: "low", angle: "web-current", sourceDate: "2025-01-01" }),
    mk({ text: "same text", source: "https://a/y", confidence: "high", angle: "docs-depth", sourceDate: "2026-01-01" }),
  ]);
  assert(out.length === 1, "merge: confidence differs");
  assert(out[0]?.confidence === "high", "survivor: highest confidence wins");
  assert(out[0]?.source === "https://a/y", "survivor: the higher-confidence claim's source wins wholesale");
  ...
  assert(out[0]?.angles?.join(",") === "web-current,docs-depth", "angles: first-appearance order even when the 2nd wins");
  ...
  assert(tied.length === 1 && tied[0]?.angle === "docs-depth", "survivor: tie → first encountered in angle order");
  assert(tied[0]?.angles?.join(",") === "docs-depth,web-current", "angles: order follows encounter");
}

{
  ...
  assert(out[1]?.angles?.join(",") === "docs-depth,codebase", "three-way: both contributing angles");
  assert(out[0]?.angles?.join(",") === "web-current" && out[2]?.angles?.join(",") === "web-current", "non-merged claims carry their single angle");
}
