...
const mkRead = (map: Record<string, string>) => (p: string): string | undefined => map[p];
...

{
  ...
  const spec = recoverOffloadedSpec(
    ...
    OFFLOAD_SCRATCH,
    OFFLOAD_REPO_ROOT,
    mkRead({ ["/tmp/elsewhere/report.md"]: OFFLOAD_SPEC }),
  );
...

{
  ...
  const spec = recoverOffloadedSpec(
    ...
    OFFLOAD_SCRATCH,
    OFFLOAD_REPO_ROOT,
    mkRead({ [`${OFFLOAD_REPO_ROOT}/tmp/issue-674/nospec.md`]: "Just some prose, no spec block here at all." }),
  );
  assert(spec === undefined, "(c) file exists but has no ## Spec → undefined (falls through to no-signal park)");
...

{
  ...
  assert(resolved?.verdict === "proceed", "(l) reconcileVerdict on the offloaded spec yields proceed");
}
