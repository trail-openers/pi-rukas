 ...
 *
 * Split (§12 file-size limit): the three #798 dual-target cases live in
 * `test-work-driver-handoff-dual-target.ts`; the shared fixtures
 * (`mkFakeForge` / `cappedState` / `fakePi`) live in
 * `lib/handoff-provenance-fixtures.ts` so neither test file carries its own
 * copy.
 */
