 ...
 * deck shows only RUNNING rows — settled runs are browsed via `/runs` — so
 * the surface is retained for the /runs finished-run integration (#836),
 * exercised today only by the smoke tests (test-runs-transcript-parse.ts).
 *
...
// d2 surface (transcript viewer below) — retained for the /runs
// finished-run integration (#836); see `openTranscriptViewer`.
