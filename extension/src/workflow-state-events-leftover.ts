/**
 * /work workflow state — worktree-leftover-handled event type.
 *
 * Issue #730 — the branch step's same-issue residue pass (worktree-
 * leftover.ts) records, per leftover worktree, WHAT it did: adopted
 * (reused a clean leftover at the cycle's own target path) or removed
 * (after preservation: salvage patch and/or a durable HEAD tag). The
 * acceptance criterion is explicit — "the branch step either reuses an
 * existing worktree knowingly or removes it first, and reports which it
 * did" — so the report is a first-class event, not just a plumb-report
 * string the operator has to parse.
 *
