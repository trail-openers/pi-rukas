...
async function runWorkDriverInner(ctx: DriverContext): Promise<DriverOutcome> {
  ...
  // distinguishes them.
  //
  // #580 — write-ahead guard: write handoffDeliveredAt BEFORE notifyAgent
  // so a crash between write and send does not cause a re-delivery on restart.
  if (final === "merged") {
    ...
    // else: already delivered — skip to avoid duplicate handoff.
  }
  return { started: true };
}
