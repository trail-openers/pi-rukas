...
- Git inspection (short output, raw): bare `git status`, `git branch`, `git worktree list`, `git rev-parse`, `git remote`, `git tag`, `git config --get`, plus read-only `git -C <path>` forms (`git -C <path> log`, `status`, `diff`, `show`, `branch --show-current`, `branch --list`, `rev-parse`, `worktree list`, `stash list`) to inspect other worktrees directly — mutations (`checkout`, `reset`, `commit`, `push`, `branch -D`, …) stay denied, and chained commands are denied as before
